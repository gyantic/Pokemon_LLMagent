...
from src.player.battle_order import BattleOrder    #インポートできてないものはまだつくってないものなので後で書く
...
class HeuristicsPlayer(Player):
    ...
    def boost_multiplier(self, state, level):
        ...
        def state_translate(self, battle: AbstractBattle):
            ...
            opponent_unfaint_inactive_pokemons = ",".join(
                opponent_unfaint_inactive_pokemons
            )
            ...
            opponent_prompt = (
                ...
                + (
                    ...
                    if len(opponent_unfaint_inactive_pokemons)
                    else "\n"
                )
                ...
                + (
                    f"Atk:{opponent_stats['atk']},"
                    if opponent_boosts["atk"] == 0
                    else f"Atk:{round(opponent_stats['atk'] * self.boost_multiplier('atk', opponent_boosts['atk']))}({opponent_boosts['atk']} stage),"
                )
                + (
                    f"Def:{opponent_stats['def']},"
                    if opponent_boosts["def"] == 0
                    else f"Def:{round(opponent_stats['def'] * self.boost_multiplier('def', opponent_boosts['def']))}({opponent_boosts['def']} stage),"
                )
                + (
                    f"Spa:{opponent_stats['spa']},"
                    if opponent_boosts["spa"] == 0
                    else f"Spa:{round(opponent_stats['spa'] * self.boost_multiplier('spa', opponent_boosts['spa']))}({opponent_boosts['spa']} stage),"
                )
                + (
                    f"Spd:{opponent_stats['spd']},"
                    if opponent_boosts["spd"] == 0
                    else f"Spd:{round(opponent_stats['spd'] * self.boost_multiplier('spd', opponent_boosts['spd']))}({opponent_boosts['spd']} stage),"
                )
                + (
                    f"Spe:{opponent_stats['spe']}"
                    if opponent_boosts["spe"] == 0
                    else f"Spe:{round(opponent_stats['spe'] * self.boost_multiplier('spe', opponent_boosts['spe']))}({opponent_boosts['spe']} stage)"
                )
            )
            ...
            if battle.opponent_active_pokemon.moves:
                for (
                    move_id,
                    opponent_move,
                ) in battle.opponent_active_pokemon.moves.items():
            ...
            active_pokemon_prompt = (
                ...
                + (
                    f"Atk:{active_stats['atk']},"
                    if active_boosts["atk"] == 0
                    else f"Atk:{round(active_stats['atk'] * self.boost_multiplier('atk', active_boosts['atk']))}({active_boosts['atk']} stage),"
                )
                + (
                    f"Def:{active_stats['def']},"
                    if active_boosts["def"] == 0
                    else f"Def:{round(active_stats['def'] * self.boost_multiplier('def', active_boosts['def']))}({active_boosts['def']} stage),"
                )
                + (
                    f"Spa:{active_stats['spa']},"
                    if active_boosts["spa"] == 0
                    else f"Spa:{round(active_stats['spa'] * self.boost_multiplier('spa', active_boosts['spa']))}({active_boosts['spa']} stage),"
                )
                + (
                    f"Spd:{active_stats['spd']},"
                    if active_boosts["spd"] == 0
                    else f"Spd:{round(active_stats['spd'] * self.boost_multiplier('spd', active_boosts['spd']))}({active_boosts['spd']} stage),"
                )
                + (
                    f"Spe:{active_stats['spe']}"
                    if active_boosts["spe"] == 0
                    else f"Spe:{round(active_stats['spe']*self.boost_multiplier('spe', active_boosts['spe']))}({active_boosts['spe']} stage)"
                )
            )
            ...
            if len(battle.available_switches) > 0:
                switch_prompt = (
                    f"You have {len(battle.available_switches)} pokemons can switch:\n"
                )
            else:
            ...
            for i, pokemon in enumerate(battle.available_switches):
                ...
                switch_prompt += (
                    ...
                    + (
                        f"Spe:{stats['spe']}" + f",Moves:{switch_move_prompt}"
                        if switch_move_prompt
                        else ""
                    )
                    + "\n"
                )
        ...
        def choose_move(self, battle: AbstractBattle):
            ...
            dump_log = {
                "prompt": system_prompt + state_prompt + "Output:",
            ...
            physical_ratio = self._stat_estimation(
                active, "atk"
            ) / self._stat_estimation(opponent, "def")
            special_ratio = self._stat_estimation(
                active, "spa"
            ) / self._stat_estimation(opponent, "spd")
